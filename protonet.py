...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            ### START CODE HERE ###
            K = torch.where(labels_support == 0)[0].shape[0]

            with torch.no_grad():
                f_support = self._network(images_support)
                c = f_support.reshape(labels_support.shape[0] // K, K, -1).mean(dim=1)

            f_query = self._network(images_query)
            logits = -((f_query.unsqueeze(1) - c.unsqueeze(0)) ** 2).sum(-1)
            loss = F.cross_entropy(logits, labels_query)
            loss_batch.append(loss)

            accuracy_query_batch.append(util.score(logits, labels_query))

            logits_support = -((f_support.unsqueeze(1) - c.unsqueeze(0)) ** 2).sum(-1)
            accuracy_support_batch.append(util.score(logits_support, labels_support))
            ### END CODE HERE ###
    ...
    def train(self, dataloader_train, dataloader_val, writer, args):
        ...
        for i_step, task_batch in enumerate(
            ...
            if i_step % VAL_INTERVAL == 0:
                ...
                writer.add_scalar("val_accuracy/support", val_accuracy_support, i_step)
                writer.add_scalar("val_accuracy/query", val_accuracy_query, i_step)

            if i_step % SAVE_INTERVAL == 0:
                self._save(i_step)

                with open(
                    f"protonet_results_{args.num_support}_{args.num_way}.npy", "wb"
                ) as f:
                    np.save(f, val_loss)
...
def main(args):
    ...
    if not args.test:
        ...
        protonet.train(dataloader_train, dataloader_val, writer, args)
    else:
        print(
