...
class MAML:
    ...
    def __init__(
        ...
        meta_parameters[f"w{NUM_CONV_LAYERS}"] = nn.init.xavier_uniform_(
            torch.empty(
                num_outputs, NUM_HIDDEN_CHANNELS, requires_grad=True, device=self.device
            )
        )
    ...
    def _inner_loop(self, images, labels, train):
        ...
        for _ in range(self._num_inner_steps + 1):
            ...
            loss = F.cross_entropy(f_theta, labels)
            gradients = autograd.grad(loss, parameters.values(), create_graph=train)

            parameters = {
                key: param - self._inner_lrs[key] * gradient
                for (key, param), gradient in zip(parameters.items(), gradients)
            }
    ...
    def _outer_step(self, task_batch, train):
        ...
        for task in task_batch:
            ...
            ### START CODE HERE ###
            parameters, accuracies, _ = self._inner_loop(
                images_support, labels_support, train
            )

            accuracies_support_batch.append(accuracies)

            f_theta_prime = self._forward(images_query, parameters)

            accuracy_query_batch.append(
                (f_theta_prime.argmax(dim=1) == labels_query).float().mean().item()
            )

            loss = F.cross_entropy(f_theta_prime, labels_query)
    ...
    def train(self, dataloader_train, dataloader_val, writer, args):
        ...
        for i_step, task_batch in enumerate(
            dataloader_train, start=self._start_train_step
        ):
            ...
            if i_step % VAL_INTERVAL == 0:
                ...
                val_accuracy_pre_adapt_support = np.mean(accuracies_pre_adapt_support)
                val_accuracy_post_adapt_support = np.mean(accuracies_post_adapt_support)
                val_accuracy_post_adapt_query = np.mean(accuracies_post_adapt_query)
                print(
                    f"Validation: "
                    f"loss: {val_loss:.3f}, "
                    f"pre-adaptation support accuracy: "
            ...
            if i_step % SAVE_INTERVAL == 0:
                ...
                with open(
                    f"maml_results_{args.num_support}_{args.num_way}_{args.num_inner_steps}_{args.inner_lr}_{args.learn_inner_lrs}.npy",
                    "wb",
                ) as f:
                    np.save(f, val_loss)
...
def main(args):
    ...
    if not args.test:
        ...
        maml.train(dataloader_train, dataloader_val, writer, args)
    else:
        print(
